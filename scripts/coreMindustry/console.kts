...
suspend fun handleInput(reader: LineReader) {
    var last = 0
    var jlineBugLastWarnAt = 0L
    var jlineBugSuppressed = 0
    while (isActive) {
        val line = try {
            runInterruptible {
            ...
            val fromJline = e.stackTrace.any { it.className.startsWith("org.jline.") }
            if (!fromJline) throw e
            val now = System.currentTimeMillis()
            if (now - jlineBugLastWarnAt >= 60_000L) {
                val suffix = if (jlineBugSuppressed > 0) " (suppressed=${jlineBugSuppressed})" else ""
                logger.warning("Console input hit JLine bug, ignore this read and continue.$suffix")
                jlineBugLastWarnAt = now
                jlineBugSuppressed = 0
            } else {
                jlineBugSuppressed++
            }
            runCatching { reader.printAbove("[warn] 控制台输入流异常，已自动恢复，请重试上一条命令") }
            continue
        }
        last = 0
...
fun start() {
    ...
    launch(Dispatchers.IO + CoroutineName("Console Reader")) {
        ...
        System.setOut(MyPrintStream {
            runCatching { reader.printAbove(AttributedString.fromAnsi(it)) }
        })
        try {
            handleInput(reader)
        } finally {

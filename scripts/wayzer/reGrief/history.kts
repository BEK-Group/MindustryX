...
sealed class Log(val uid: String?, val desc: PlaceHoldString) {
    ...
    private fun resolvePublicIdentity(uid: String): Pair<String, String> {
        val online = Groups.player.find { it.uuid() == uid }
        if (online != null) {
            val publicName = Strings.stripColors(online.name).ifBlank { "未知玩家" }
            val publicShortId = PlayerData[online].shortId
            return publicName to publicShortId
        }
        val masked = uid.filter { it.isLetterOrDigit() }.take(6).ifBlank { "unknown" }
        return "离线玩家" to masked
    }
    ...
    fun descLog(descPrefix: String = ""): PlaceHoldString {
        return if (uid == null) {
            ...
            val (publicName, publicShortId) = resolvePublicIdentity(uid)
            "[red]{time HH:mm:ss}[]-[yellow]{name}[yellow]({shortID})[white]{descPrefix}{desc}".with(
                "time" to time,
                "descPrefix" to descPrefix,
                "desc" to desc,
                "name" to publicName,
                "shortID" to publicShortId
            )
        }
    }
}

...
val overrideName = mutableMapOf<String, String>()
fun setNameOverride(uuid: String, name: String) {
    overrideName[uuid] = name
}

fun clearNameOverride(uuid: String) {
    overrideName.remove(uuid)
}

fun clearAllNameOverride() {
    overrideName.clear()
}
...
fun Player.updateName() {
    name = "[white]{player.prefix}[]{name}[white]{player.suffix}".with(
        "player" to this,
        "name" to (overrideName[uuid()] ?: realName[uuid()] ?: "NotInit")
    ).toString()
}
